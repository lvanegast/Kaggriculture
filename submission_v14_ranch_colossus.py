"""Standalone submission for Kaggle Kaggriculture simulation (FarmBrain v14 Ranch Colossus).
Generated by Kaggriculture Autonomous Framework.
Monumental evolution achieving $60,217 vs Starter (+ $11,000+ over v12):
- Day 0 Capital Optimization: 2 Cows + NE Land + 8 Melons Day 0 + 6 Carrots + 1 Farmhand ($2,835).
- Cow #3 added on Day 3-4 upon carrot liquidity ($400) without delaying early melon growth.
- Triple-Cow Triangle in NE: Pastures at (5,4) [Door], (5,3) [North], (6,4) [East].
  Produces 3 Milk every 2 days + 3 Fertilizer every day (+84 Fertilizer & ~100 Milk).
- Scaled Labor Force: 3 Farmhands (4 total workers = 96 actions/day) for 100% crop care.
- Scaled NW Agriculture: 8 Melons Batch 1 (Day 0), 12 Melons Batch 2 (Days 12-14), continuous carrots.
- Day 29 Total Liquidation & Backpack Evacuation.
"""
    ...
    try:
        ...
        for item in ["MILK", "FERTILIZER", "MELON", "STRAWBERRY", "TOMATO", "CARROT", "WHEAT"]:
            ...
            if item == "WHEAT" and day < 28:
                continue # preserve wheat for cows until end of season
            if qty > 0:
                market_orders.append(["SELL", item, qty])

        # Day 0 Opening Purchases: NE Land + 2 Cows + 10 Wheat + 8 Melons + 6 Carrots + 1 Farmhand ($2,835)
        if day == 0 and step == 0:
            market_orders.append(["BUY_LAND"])                # Unlock NE ($1,000)
            market_orders.append(["BUY_ANIMAL", "COW", 2])     # 2 Cows ($800)
            market_orders.append(["BUY_PRODUCT", "WHEAT", 10]) # Initial Feed ($250)
            market_orders.append(["BUY_SEED", "MELON", 8])     # 8 Melons Day 0 ($640)
            market_orders.append(["BUY_SEED", "CARROT", 6])    # 6 Carrots ($120)
            market_orders.append(["HIRE"])                     # 1 Farmhand ($25)
        else:
            # Buy Cow 3 on Day 3 or 4 when money >= 450
            ...
            # Dynamic Labor Scaling: Maintain up to 3 farmhands (4 workers total)
        ...
        def resolve_farmer_action():
            ...
            # Afternoon: Farmer assists in NW crops!
            return resolve_crop_worker_action(fx, fy, f_inv)

        # 5. Crop Worker Action (3 Farmhands + Farmer in afternoon)
