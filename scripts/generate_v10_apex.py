"""Generates and verifies submission_v10_apex.py."""

from pathlib import Path
import kaggle_environments as ke

V10_CODE = '''"""Standalone submission for Kaggle Kaggriculture simulation (FarmBrain v10.0 Grandmaster Apex).
...
def main():
    p = Path("submission_v10_apex.py")
    p.write_text(V10_CODE, encoding="utf-8")
    print(f"Written {p}")

    env_s = ke.make("kaggriculture")
    env_s.run([str(p), "starter"])
    print(f"Vs Starter: Reward=${env_s.steps[-1][0]['reward']:,.2f}")

    env_v4 = ke.make("kaggriculture")
    env_v4.run([str(p), "submission_v4_megacluster.py"])
    r_sub = env_v4.steps[-1][0]["reward"]
    r_opp = env_v4.steps[-1][1]["reward"]
    print(f"Vs V4 (P0): Sub=${r_sub:,.2f} vs V4=${r_opp:,.2f} (Lead=${r_sub - r_opp:+,.2f})")

    env_v4_2 = ke.make("kaggriculture")
    env_v4_2.run(["submission_v4_megacluster.py", str(p)])
    r_opp2 = env_v4_2.steps[-1][0]["reward"]
    r_sub2 = env_v4_2.steps[-1][1]["reward"]
    print(f"Vs V4 (P1): Sub=${r_sub2:,.2f} vs V4=${r_opp2:,.2f} (Lead=${r_sub2 - r_opp2:+,.2f})")
