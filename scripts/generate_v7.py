"""Generates and verifies submission_v7_adaptive.py."""

from pathlib import Path
import kaggle_environments as ke

SUBMISSION_V7 = '''"""Standalone submission for Kaggle Kaggriculture simulation (FarmBrain v7.0 Adaptive).
...
def generate_and_verify():
    out_path = Path("submission_v7_adaptive.py")
    out_path.write_text(SUBMISSION_V7, encoding="utf-8")
    print(f"Generated {out_path}")

    # Verify against Starter
    env = ke.make("kaggriculture")
    env.run([str(out_path), "starter"])
    r_sub = env.steps[-1][0]["reward"]
    print(f"Verified vs Starter: Status={env.steps[-1][0]['status']}, Reward=${r_sub:,.2f}")

    # Verify against V4 Megacluster
    env2 = ke.make("kaggriculture")
    env2.run([str(out_path), "submission_v4_megacluster.py"])
    r_sub2 = env2.steps[-1][0]["reward"]
    r_opp2 = env2.steps[-1][1]["reward"]
    print(f"Verified vs V4: Sub=${r_sub2:,.2f} vs Opp=${r_opp2:,.2f} (Lead=${r_sub2-r_opp2:+,.2f})")
