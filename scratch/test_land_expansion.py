...
def create_expansion_agent(buy_land=True):
    def agent(obs, config=None):
        ...
        # 2. Buy Land Expansion if enabled
        if buy_land and "NE" not in unlocked and day <= 10 and money >= 2000:
        ...
        # 4. Seed Purchasing
        s_count = 6 if "NE" in unlocked else 4
        m_count = 16 if "NE" in unlocked else 12
        c_count = 4
        ...
        if day <= 3 and s_seeds < s_count and money >= 100:
        ...
        # Candidates including NE quadrant tiles adjacent to shed
        candidates = [
        ...
        if "NE" in unlocked:
            ne_candidates = [(5, 4), (5, 3), (5, 2), (6, 4), (6, 3), (5, 1)]
            candidates = candidates + ne_candidates
        ...
        def resolve_worker_action(wx, wy, worker_inv):
            ...
            if wy < len(tiles) and wx < len(tiles[0]):
                ...
                if isinstance(ct, dict):
                    ...
                    if (wx, wy) in s_slots and s_seeds > 0 and day <= 3:
                        claimed_targets.add((wx, wy))
            ...
            for cx, cy in cluster:
                ...
                if isinstance(t, dict):
                    ...
                    if (cx, cy) in s_slots and s_seeds > 0 and day <= 3:
...
def test():
    no_land = create_expansion_agent(buy_land=False)
    with_land = create_expansion_agent(buy_land=True)

    print("--- TESTING BUY_LAND EXPANSION ---")
    env1 = ke.make("kaggriculture")
    env1.run([no_land, "starter"])
    print(f"Without Land vs Starter: ${env1.steps[-1][0]['reward']:,.2f}")

    env2 = ke.make("kaggriculture")
    env2.run([with_land, "starter"])
    print(f"With Land vs Starter:    ${env2.steps[-1][0]['reward']:,.2f}")

    env3 = ke.make("kaggriculture")
    env3.run([with_land, "submission_v4_megacluster.py"])
    print(f"With Land vs V4:         ${env3.steps[-1][0]['reward']:,.2f} vs V4: ${env3.steps[-1][1]['reward']:,.2f}")
