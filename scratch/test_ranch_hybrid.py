"""
Complete prototype of FarmBrain v12 Ranch Apex Master.
Combines 2-Cow NE Ranch + NW Crop Engine.
"""
...
def agent(obs, config=None):
    try:
        ...
        for item in ["MILK", "FERTILIZER", "MELON", "STRAWBERRY", "TOMATO", "CARROT", "WHEAT"]:
            qty = shed.get(item, 0)
            if item == "WHEAT":
                continue # don't sell wheat needed for cows
            if qty > 0:
        ...
        if day == 0 and step == 0:
            ...
            market_orders.append(["BUY_PRODUCT", "WHEAT", 10]) # Wheat ($250)
            ...
            # Wheat replenishment for cows
            if shed.get("WHEAT", 0) < 6 and money >= 150:
                market_orders.append(["BUY_PRODUCT", "WHEAT", 6])

            # Maintain 2 farmhands (3 workers)
        ...
        # NE Ranch cluster
        shed_ne = (5, 4) # Pasture 1 & Shed Access!
        ...
        def resolve_farmer_action():
            # Day 0 Exact Ranch Setup Sequence
            if day == 0:
                ...
                if step == 17: return ["WEST"] # back to (4,4)
                # Steps 18-23: Help in NW!
            ...
            if ranch_active:
                ...
                if (fx, fy) == (5, 4):
                    ...
                    if pt1 and isinstance(pt1, dict) and not pt1.get("cared_today"):
                        return ["CARE"]
                    if cow2_needs:
                        return ["NORTH"] # go tend cow 2 at (5,3)
                    if has_produce:
