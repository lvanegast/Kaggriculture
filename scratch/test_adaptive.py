...
def create_adaptive_agent():
    def agent(obs, config=None):
        ...
        # OPPONENT PROFILING:
        # Check opponent tiles to see if they are melon rushers
        ...
        # If opponent is planting melons (>= 4), ladder is saturated with melons:
        # We plant 8 Melons, 4 Strawberries, 4 Carrots (Anti-crash diversified portfolio)
        # If opponent has few/no melons (< 4), melon market is unsaturated:
        # We plant 14 Melons, 0 Strawberries, 4 Carrots (Monopoly Melon Cash Maximizer!)
        if opp_melons >= 4:
            num_melons = 8
            num_strawberries = 4
            num_carrots = 4
        ...
        # 2. Market Phase: Labor Scaling (Hire up to 2 hands)
...
def test_adaptive():
    ag = create_adaptive_agent()

    print("--- TESTING ADAPTIVE VS V4 MEGACLUSTER (Melon Rusher) ---")
    env1 = ke.make("kaggriculture")
    env1.run([ag, "submission_v4_megacluster.py"])
    r1 = env1.steps[-1][0]["reward"]
    o1 = env1.steps[-1][1]["reward"]
    print(f"Adaptive P0 vs V4 P1: Adaptive=${r1:,.2f} | V4=${o1:,.2f} | Diff=${(r1-o1):+,.2f}")

    print("\n--- TESTING ADAPTIVE VS STARTER (Non-melon agent) ---")
    env2 = ke.make("kaggriculture")
    env2.run([ag, "starter"])
    r2 = env2.steps[-1][0]["reward"]
    o2 = env2.steps[-1][1]["reward"]
    print(f"Adaptive P0 vs Starter: Adaptive=${r2:,.2f} | Starter=${o2:,.2f}")
