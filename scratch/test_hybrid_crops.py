...
def create_hybrid_agent(m_count=6, s_count=4, t_count=4, c_count=2):
    def agent(obs, config=None):
        ...
        if day <= 2 and t_seeds < t_count and money >= 50:
            nt = min(t_count - t_seeds, int(money // 50))
            if nt > 0: market_orders.append(["BUY_SEED", "TOMATO", nt])
        ...
        # Ongoing crops closest to shed
        s_slots = candidates[:s_count]
        t_slots = candidates[s_count : s_count + t_count]
        m_slots = candidates[s_count + t_count : s_count + t_count + m_count]
        c_slots = candidates[s_count + t_count + m_count : s_count + t_count + m_count + c_count]

        cluster = [shed_access] + s_slots + t_slots + m_slots + c_slots
        ...
        def resolve_worker_action(wx, wy, worker_inv):
            ...
            if wy < len(tiles) and wx < len(tiles[0]):
                ...
                if isinstance(ct, dict):
                    ...
                    elif (wx, wy) in t_slots and t_seeds > 0 and day <= 2:
                        claimed_targets.add((wx, wy))
                        return ["PLANT", "TOMATO"]
            ...
            for cx, cy in cluster:
                ...
                if isinstance(t, dict):
                    ...
                    elif (cx, cy) in t_slots and t_seeds > 0 and day <= 2:
                        needed.append(((cx, cy), 4))
...
def run():
    print("--- HYBRID BENCHMARKS ---")
    hybrids = [
        ("Balanced_8M_4S_4C", 8, 4, 0, 4),
        ("Hybrid_6M_4S_4T_2C", 6, 4, 4, 2),
        ("Perennial_4M_6S_4T_2C", 4, 6, 4, 2),
        ("TriplePerennial_2M_6S_6T_2C", 2, 6, 6, 2),
    ]
    for name, m, s, t, c in hybrids:
        ag = create_hybrid_agent(m, s, t, c)
        ...
        print(f"{name:28s} | vs V4: Ag=${r1:,.2f} vs V4=${v4_r:,.2f} (diff=${r1-v4_r:+,.2f}) | vs Starter: ${r2:,.2f}")
