...
def create_v11_agent():
    """FarmBrain v11: Direct Evolution of v7 (which reached 431 Elo on Kaggle).
    Improvements over v7:
    1. Safe Day 0 anchor: 4 Strawberries + 4 Carrots + 6 Melons initially.
    2. Smart expansion at Day 2: if opponent has < 4 melons, add 6 more melons.
    3. Strict anti-waste seed cutoff: No melons after Day 14, no carrots after Day 25.
    4. Day 29 Emergency Evacuation: all workers drop backpack produce into shed.
    5. Retains the 2 Farmhand (3 worker) labor model that proved optimal on Kaggle.
    """
    def agent(obs, config=None):
        try:
            ...
            # 1. Opponent Profiling (Reliable from Day 1 onwards)
            ...
            # If opponent rushes melons (>=3), defend with 8 Melons + 4 Strawberries + 4 Carrots
            # If opponent is passive (<3), capitalize with 14 Melons + 0 Strawberries + 4 Carrots
            if day >= 2 and opp_melons >= 3:
                num_melons = 8
                num_strawberries = 4
                num_carrots = 4
            elif day >= 2 and opp_melons < 3:
                num_melons = 14
                num_strawberries = 0
                num_carrots = 4
            else:
                # Day 0-1: Safe balanced starter
                num_melons = 8
                num_strawberries = 4
                num_carrots = 4
            ...
            # 3. Labor: 2 Farmhands (proven optimal congestion-free ratio)
            if day <= 25 and len(hands_pos) < 2 and money >= 25:
                market_orders.append(["HIRE"])

            # 4. Seed Purchasing with Strict Anti-Waste Cutoffs
            s_seeds = seeds.get("STRAWBERRY", 0)
            m_seeds = seeds.get("MELON", 0)
            c_seeds = seeds.get("CARROT", 0)

            # Strawberries: only Days 0-2 (need 10 days to yield)
            ...
            # Melons: Batch 1 (Days 0-2), Batch 2 (Days 12-14). Never buy after Day 14!
            ...
            # Carrots: need 3 days. Never buy after Day 25!
...
def benchmark():
    v11 = create_v11_agent()

    print("--- BENCHMARK V11 VS V7 (Our current best, 431 Elo) ---")
    # Duel 1: V11 as P0 vs V7 as P1
    env1 = ke.make("kaggriculture")
    env1.run([v11, "submission_v7_adaptive.py"])
    r11 = env1.steps[-1][0]["reward"]
    r7 = env1.steps[-1][1]["reward"]
    print(f"Duel 1: V11 P0=${r11:,.2f} vs V7 P1=${r7:,.2f} (V11 diff=${r11 - r7:+,.2f})")

    # Duel 2: V7 as P0 vs V11 as P1
    env2 = ke.make("kaggriculture")
    env2.run(["submission_v7_adaptive.py", v11])
    r7_2 = env2.steps[-1][0]["reward"]
    r11_2 = env2.steps[-1][1]["reward"]
    print(f"Duel 2: V7 P0=${r7_2:,.2f} vs V11 P1=${r11_2:,.2f} (V11 diff=${r11_2 - r7_2:+,.2f})")

    # Vs Starter
    env3 = ke.make("kaggriculture")
    env3.run([v11, "starter"])
    print(f"V11 vs Starter: ${env3.steps[-1][0]['reward']:,.2f}")

    # Vs V4
    env4 = ke.make("kaggriculture")
    env4.run([v11, "submission_v4_megacluster.py"])
    r11_4 = env4.steps[-1][0]["reward"]
    r4 = env4.steps[-1][1]["reward"]
    print(f"V11 vs V4: V11=${r11_4:,.2f} vs V4=${r4:,.2f} (V11 diff=${r11_4 - r4:+,.2f})")
